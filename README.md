...
| Skill 商店 | 免费开源 Skill 目录（[easycc-skills](https://github.com/duliangkuan/easycc-skills)），一键安装到 `~/.claude/skills/`，欢迎 PR 投稿 |
| Memory | Claude Code 全局记忆（CLAUDE.md）可视化：漂亮渲染、直接编辑、文件变化实时同步 |
| 设置 · 接入 | 服务商模板（DeepSeek 官方 / 硅基流动 / 自定义）+ 主/小模型 + 与 CC 同链路的连接检测 |
