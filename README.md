...
**中国大陆的普通人，三分钟用上 Claude Code。** Windows / macOS 双平台。
...
从 [Releases](../../releases) 或官网 [fy.dufengyun.xyz](https://fy.dufengyun.xyz) 下载：

| 平台 | 安装包 | 说明 |
|---|---|---|
| Windows 10/11 | `EasyCC-Setup-*.exe` | 双击安装 |
| macOS（Apple 芯片 M 系列） | `EasyCC-*-arm64.dmg` | 拖进 Applications |
| macOS（Intel） | `EasyCC-*-x64.dmg` | 拖进 Applications |

> **macOS 首次打开提示「已损坏/无法验证开发者」**：这是未付费签名的开源应用的正常现象。
> 终端执行 `xattr -cr /Applications/EasyCC.app` 后再打开即可（或右键 App → 打开）。
...
npm run dist:mac    # 打包 macOS dmg（需在 macOS 上跑；仓库 CI 已自动打双平台）
# 自检模式
